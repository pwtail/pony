...
class TestRawSQL(unittest.TestCase):
    ...
    def test_11(self):
        ...
        dates = select(raw_sql("(p.dob)", result_type=date) for p in Person).order_by(
            lambda: p.id
        )[:]
        self.assertEqual(
    ...
    def test_15(self):
        ...
        persons = Person.select(
            lambda p: raw_sql("UPPER(p.name) || $x") == raw_sql("UPPER($y || '123')")
        )[:]
    ...
    @raises_exception(
        TranslationError,
        "Expression `raw_sql(p.name)` cannot be translated into SQL "
