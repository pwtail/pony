...
class TestDiag(unittest.TestCase):
    @raises_exception(
        TypeError, "`interleave` option cannot be specified for Set attribute Foo.x"
    )
    def test_1(self):
        db = Database()
    ...
    @raises_exception(
        TypeError, "`interleave` option value should be True, False or None. Got: 'yes'"
    )
    def test_2(self):
        db = Database()
    ...
    @raises_exception(
        TypeError, "only one attribute may be marked as interleave. Got: Foo.x, Foo.y"
    )
    def test_3(self):
        db = Database()
    ...
    @raises_exception(
        TypeError, "Interleave attribute should be part of relationship. Got: Foo.x"
    )
    def test_4(self):
        db = Database()
