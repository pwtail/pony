...
class TestSQLTranslator2(unittest.TestCase):
    ...
    def test_distinct3(self):
        q = select(
            d
            for d in Department
            if len(s for c in d.courses for s in c.students) > len(s for s in Student)
        )
        self.assertEqual(q[:], [])
    ...
    def test_not_null1(self):
        q = select(
            g
            for g in Group
            if "123-45-67" not in g.students.tel and g.dept == Department[1]
        )
    ...
    def test_not_null2(self):
        q = select(
            g
            for g in Group
            if "John" not in g.students.name and g.dept == Department[1]
        )
    ...
    def test_chain_of_attrs_inside_for1(self):
        result = set(
            select(s for d in Department if d.number == 2 for s in d.groups.students)
        )
    ...
    def test_non_entity_result1(self):
        result = select(
            (s.name, s.group.number) for s in Student if s.name.startswith("J")
        )[:]
        self.assertEqual(sorted(result), [("Jing Xia", 102), ("John Smith", 101)])
    ...
    def test_non_entity_result4(self):
        ...
        self.assertEqual(result, {"Matthew Reed", "Maria Ionescu"})
    ...
    def test_non_entity_result5(self):
        ...
        self.assertEqual(
            sorted(result),
            [
                (Group[101], date(1989, 2, 5)),
                (Group[101], date(1990, 11, 26)),
                (Group[101], date(1991, 3, 20)),
            ],
        )
    ...
    def test_non_entity_result6(self):
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (Course["Linear Algebra", 1], Student[1]),
                    (Course["Linear Algebra", 1], Student[2]),
                    (Course["Web Design", 1], Student[1]),
                    (Course["Web Design", 1], Student[2]),
                ]
            ),
        )
    ...
    def test_non_entity7(self):
        result = set(
            select(
                ...
                for s in Student
                if (s.name, s.dob)
                not in ((s2.name, s2.dob) for s2 in Student if s.group.number == 101)
            )
        )
        self.assertEqual(result, {Student[4], Student[5], Student[6], Student[7]})
    ...
    @raises_exception(
        ...
        "Incomparable types 'int' and 'Set of Student' in expression: g.number == g.students",
    )
    def test_incompartible_types(self):
        select(g for g in Group if g.number == g.students)

    @raises_exception(
        TranslationError, "External parameter 'x' cannot be used as query result"
    )
    ...
    @raises_exception(
        TypeError, "Use select(...) function or Group.select(...) method for iteration"
    )
    ...
    @raises_exception(
        ExprEvalError,
        "`db.FooBar` raises AttributeError: 'Database' object has no attribute 'FooBar'",
    )
    ...
    def test_keyargs1(self):
        result = set(
            select(s for s in Student if s.dob < date(year=1990, month=10, day=20))
        )
    ...
    def test_query_as_string1(self):
        ...
        self.assertEqual(
            result,
            {Student[1], Student[2], Student[4], Student[5], Student[6], Student[7]},
        )
    ...
    def test_str_subclasses(self):
        result = select(
            ...
            for g in d.groups
            for c in d.courses
            if g.number == 106 and c.name.startswith("T")
        )[:]
    ...
    def test_bool5(self):
        x = True
        result = set(
            select(s for s in Student if s.phd == True and (False or (True and x)))
        )
        self.assertEqual(result, {Student[1], Student[2]})
    ...
    def test_bool6(self):
        ...
        result = list(
            select(
                s for s in Student if s.phd == (False or (True and x)) and s.phd is True
            )
        )
        self.assertEqual(result, [])
