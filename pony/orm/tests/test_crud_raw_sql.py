...
from pony.orm.tests import only_for, setup_database, teardown_database
...
class TestCrudRawSQL(unittest.TestCase):
    ...
    def setUp(self):
        with db_session:
            ...
            db.insert(
                Student, id=1, name="A", age=30, group_dept=44, group_grad_year=1999
            )
            db.insert(
                Student, id=2, name="B", age=25, group_dept=44, group_grad_year=1999
            )
            db.insert(
                Student, id=3, name="C", age=20, group_dept=44, group_grad_year=1999
            )
        rollback()
    ...
    @raises_exception(
        TypeError,
        "The first positional argument must be lambda function or its text source. Got: 123",
    )
    def test4(self):
    ...
    def test6(self):
        ...
        y = 30
        cursor = db.execute(
            "select name, 'abc$$def%' from Student where id = $x and age = $y"
        )
