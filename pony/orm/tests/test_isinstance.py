...
class TestVolatile(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with db_session:
            ...
            g = Group(number=123)
            prof = Professor(
                name="Professor1", salary=1000, position="position1", ssn="SSN5"
            )
            a1 = Assistant(
                name="Assistant1", group=g, salary=100, ssn="SSN4", mentor=prof
            )
            a2 = Assistant(
                name="Assistant2", group=g, salary=200, ssn="SSN6", mentor=prof
            )
    ...
    def test_5(self):
        ...
        self.assertEqual(
            set(q), {"Student1", "Student2", "Assistant1", "Assistant2", "Professor1"}
        )

    @db_session
    def test_6(self):
        q = select(
            p.name for p in Person if isinstance(p, Student) and isinstance(p, Teacher)
        )
