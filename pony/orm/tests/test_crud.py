...
class TestCRUD(unittest.TestCase):
    ...
    @raises_exception(
        ValueError,
        "Value type for attribute Group.id must be int. Got string 'not a number'",
    )
    ...
    @raises_exception(
        TypeError,
        "Attribute Student.group must be of Group type. Got: datetime.date(2011, 1, 1)",
    )
    ...
    @raises_exception(
        TypeError,
        "Invalid number of columns were specified for attribute Student.group. Expected: 1, got: 2",
    )
