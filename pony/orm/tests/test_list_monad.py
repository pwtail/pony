...
class TestListMonad(unittest.TestCase):
    ...
    def test_not_in_simple(self):
        q = select(
            s.id for s in Student if s.name not in ("John Smith", "Matthew Reed")
        )
    ...
    def test_in_composite(self):
        q = select(
            c.name
            for c in Course
            if (c.name, c.semester)
            in [("Web Design", 1), ("Thermodynamics", 2), ("Theology", 3)]
        )
    ...
    def test_not_in_composite(self):
        q = select(
            ...
            for c in Course
            if (c.name, c.semester)
            not in [("Web Design", 1), ("Thermodynamics", 2), ("Theology", 3)]
        )
        self.assertEqual(
            set(q),
            {
