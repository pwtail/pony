...
import threading
import unittest
from contextlib import ContextDecorator, closing, contextmanager
...
from pony.orm import Database, db_session, delete, rollback
...
class SqlServerContext(DBContext):
    ...
    def get_conn_string(self, db=None):
        s = "DSN=MSSQLdb;SERVER=mssql;UID=sa;PWD=pass;"
        if db:
            s += "DATABASE=%s" % db
        return s
