...
class TestDistinct(unittest.TestCase):
    ...
    def test_group_by(self):
        result = set(
            select((s.age, sum(s.scholarship)) for s in Student if s.scholarship > 0)
        )
    ...
    def test_group_by_having(self):
        result = set(
            select(
                (s.age, sum(s.scholarship)) for s in Student if sum(s.scholarship) < 300
            )
        )
    ...
    def test_aggregation_no_group_by_2(self):
        result = set(
            select(
                (sum(s.scholarship), min(s.scholarship)) for s in Student if s.age < 23
            )
        )
    ...
    def test_aggregation_no_group_by_3(self):
        result = set(
            select(
                (sum(s.scholarship), min(s.scholarship))
                for s in Student
                for g in Group
