...
from pony.orm.ormtypes import Json, TrackedDict, TrackedList, TrackedValue
...
class TestJson(unittest.TestCase):
    ...
    def test_list_append(self):
        ...
        with db_session:
            ...
            self.assertListEqual(
                p.info["colors"], ["Gold", "Silver", "Space Gray", "White"]
            )
    ...
    def test_equal_json_1(self):
        with raises_if(
            ...
            p = get(
                p for p in Product if p.info["os"] == {"type": "iOS", "version": "8"}
            )
            self.assertTrue(p)
    ...
    def test_equal_json_2(self):
        with raises_if(
            ...
            p = get(
                p
                for p in Product
                if p.info["os"] == Json({"type": "iOS", "version": "8"})
            )
            self.assertTrue(p)
    ...
    @raises_exception(
        TypeError,
        "Incomparable types 'Json' and 'list' in expression: p.info['colors'] == ['Gold']",
    )
    ...
    @raises_exception(
        TypeError,
        "Incomparable types 'Json' and 'list' in expression: p.info['colors'] == []",
    )
    ...
    def test_wildcard_path_1(self):
        with raises_if(
            self,
            db.provider.dialect not in ("Oracle", "MySQL"),
            TranslationError,
    ...
    def test_var_in_json(self):
        with raises_if(
            ...
            db.provider.dialect == "Oracle",
            TypeError,
            "For `key in JSON` operation Oracle supports literal key values only, "
