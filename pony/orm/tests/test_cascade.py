...
class TestCascade(unittest.TestCase):
    ...
    def assert_on_delete(self, table_name, value):
        ...
        if pony.__version__ >= "0.9":
            ...
            self.assertIn(
                "ON DELETE %s" % value, list(fkeys.values())[0].get_create_command()
            )
    ...
    @raises_exception(
        TypeError,
        "'cascade_delete' option cannot be set for attribute Group.persons, because reverse attribute Person.group is collection",
    )
    def test_4(self):
        db = self.db = Database()
    ...
    @raises_exception(
        TypeError,
        "'cascade_delete' option cannot be set for both sides of relationship (Person.group and Group.persons) simultaneously",
    )
    def test_5(self):
        db = self.db = Database()
