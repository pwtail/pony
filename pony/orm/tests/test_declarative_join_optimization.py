...
class TestM2MOptimization(unittest.TestCase):
    ...
    def test5(self):
        ...
        self.assertEqual(
            Group._table_ in flatten(q._translator.sqlquery.from_ast), True
        )
    ...
    def test7(self):
        q = select(
            s
            for s in Student
            if sum(c.credits for c in Course if s.group.dept == c.dept) > 10
        )
        objects = q[:]
        ...
        self.assertEqual(
            q._translator.sqlquery.from_ast,
            [
                "FROM",
                ["s", "TABLE", student_table_name],
                [
                    "group",
                    "TABLE",
                    group_table_name,
