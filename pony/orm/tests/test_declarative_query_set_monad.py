...
class TestQuerySetMonad(unittest.TestCase):
    ...
    def test_len_2(self):
        result = set(
            select(g for g in Group if len(s for s in Student if s.group == g) > 1)
        )
        self.assertEqual(result, {Group[1]})

    def test_len_3(self):
        result = set(
            select(g for g in Group if len(s.name for s in Student if s.group == g) > 1)
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_count_2(self):
        result = set(
            select(g for g in Group if select(s.name for s in g.students).count() > 1)
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_count_3a(self):
        result = set(
            select(s for s in Student if select(c for c in s.courses).count() > 1)
        )
        self.assertEqual(result, {Student[2], Student[3]})
    ...
    def test_count_3b(self):
        result = set(
            select(
                s
                for s in Student
                if select(c for c in s.courses).count(distinct=False) > 1
            )
        )
        self.assertEqual(result, {Student[2], Student[3]})
    ...
    @raises_exception(
        TypeError,
        "`distinct` value should be True or False. Got: s.name.startswith('P')",
    )
    ...
    def test_sum_7(self):
        result = set(
            select(
                g
                for g in Group
                if sum(s.scholarship for s in Student if s.group == g) > 500
            )
        )
    ...
    def test_sum_8(self):
        result = set(
            select(
                g
                for g in Group
                if select(s.scholarship for s in g.students).sum() > 200
            )
        )
    ...
    def test_sum_9(self):
        result = set(
            select(
                g
                for g in Group
                if select(s.scholarship for s in g.students).sum(distinct=True) > 200
            )
        )
        self.assertEqual(result, {Group[2]})
        self.assertTrue("DISTINCT" in db.last_sql)
    ...
    def test_min_1(self):
        result = set(
            select(
                g for g in Group if min(s.name for s in Student if s.group == g) == "S1"
            )
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_min_3(self):
        result = set(
            select(
                g for g in Group if select(s.scholarship for s in g.students).min() == 0
            )
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_max_1(self):
        result = set(
            select(
                g
                for g in Group
                if max(s.scholarship for s in Student if s.group == g) > 100
            )
        )
        self.assertEqual(result, {Group[2]})
    ...
    def test_max_3(self):
        result = set(
            select(
                g
                for g in Group
                if select(s.scholarship for s in g.students).max() == 100
            )
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_avg_1(self):
        result = select(
            g
            for g in Group
            if avg(s.scholarship for s in Student if s.group == g) == 50
        )[:]
        self.assertEqual(result, [Group[1]])
    ...
    def test_avg_2(self):
        result = set(
            select(
                g
                for g in Group
                if select(s.scholarship for s in g.students).avg() == 50
            )
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_avg_5(self):
        result = set(
            select(
                g
                for g in Group
                if select(s.scholarship for s in g.students).avg(distinct=True) == 50
            )
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_exists_1(self):
        result = set(
            select(g for g in Group if exists(s for s in g.students if s.age < 23))
        )
        self.assertEqual(result, {Group[1]})
    ...
    def test_negate(self):
        result = set(
            select(
                g for g in Group if not (s.scholarship for s in Student if s.group == g)
            )
        )
        self.assertEqual(result, set())
    ...
    def test_no_conditions_2(self):
        students = set(
            select(
                s
                for s in Student
                if s.scholarship == max(s.scholarship for s in Student)
            )
        )
    ...
    def test_hint_join_1(self):
        result = set(
            select(
                s
                for s in Student
                if JOIN(s.group in select(g for g in Group if g.id < 2))
            )
        )
    ...
    def test_hint_join_2(self):
        result = set(
            select(
                s
                for s in Student
                if JOIN(s.group not in select(g for g in Group if g.id < 2))
            )
        )
    ...
    def test_hint_join_3(self):
        result = set(
            select(
                ...
                if JOIN(
                    s.scholarship
                    in select(s.scholarship + 100 for s in Student if s.name != "S2")
                )
            )
        )
    ...
    def test_hint_join_4(self):
        result = set(
            select(g for g in Group if JOIN(g in select(s.group for s in g.students)))
        )
    ...
    def test_group_concat_3(self):
        result = select(
            s for s in Student if s.name in group_concat(s.name for s in Student)
        )[:]
    ...
    def test_select_from_select_4(self):
        p, q = 50, 400
        result = select(
            x.id for x in (s for s in Student if s.scholarship > p) if x.scholarship < q
        )[:]
        self.assertEqual(set(result), {2})
    ...
    def test_select_from_select_5(self):
        ...
        result = select(
            x.id
            for x in select(s for s in Student if s.scholarship > 0)
    ...
    @raises_exception(
        TranslationError,
        'Too many values to unpack "for a, b in select(s for ...)" (expected 2, got 1)',
    )
    ...
    @raises_exception(
        NotImplementedError,
        "Please unpack a tuple of (s.name, s.group) in for-loop "
    ...
    @raises_exception(
        TranslationError,
        'Not enough values to unpack "for x, y in '
        'select(s.name, s.group, s.scholarship for ...)" (expected 2, got 3)',
    )
    def test_select_from_select_9(self):
        query = select(
            (s.name, s.group, s.scholarship) for s in Student if s.scholarship > 0
        )
