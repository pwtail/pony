...
class TestIndexes(unittest.TestCase):
    ...
    def test_17(self):  # named single-column PrimaryKey
        db = self.db

        class Person(db.Entity):
            id = PrimaryKey(int, name="pk_person")
            name = Required(str)

        db.generate_mapping(create_tables=True)
        table = db.schema.tables[Person._table_]
        self.assertEqual(table.pk_index.name, "pk_person")
        script = db.schema.generate_create_script()
        dialect = db.provider.dialect
        if dialect == "MySQL":
            constraint_sql = "CONSTRAINT `pk_person` PRIMARY KEY (`id`)"
    ...
    def test_18(self):  # named m2m table primary key
        db = self.db

        class Person(db.Entity):
            name = Required(str)
            tags = Set("Tag", reverse="persons", pk_name="pk_person_tag")
        ...
        dialect = db.provider.dialect
        if dialect == "MySQL":
            constraint_sql = "CONSTRAINT `pk_person_tag` PRIMARY KEY"
        elif dialect in ("SQLite", "PostgreSQL"):
            constraint_sql = 'CONSTRAINT "pk_person_tag" PRIMARY KEY'
        elif dialect == "Oracle":
            constraint_sql = 'CONSTRAINT "PK_PERSON_TAG" PRIMARY KEY'
    ...
    def test_19(self):  # conflicting pk_name values on the two m2m sides
        db = self.db

        with self.assertRaises(MappingError):
            class Person(db.Entity):
                name = Required(str)
                tags = Set("Tag", reverse="persons", pk_name="pk_one")

            class Tag(db.Entity):
                name = Required(str)
                persons = Set(Person, pk_name="pk_two")
...
class TestIndexOptionsPostgreSQL(unittest.TestCase):
    ...
    def test_named_auto_pk(self):
        db = self.db

        class Person(db.Entity):
            id = PrimaryKey(int, auto=True, name="pk_person")
            name = Required(str)

        db.generate_mapping(create_tables=True)
        script = db.schema.generate_create_script()
        self.assertIn('CONSTRAINT "pk_person" PRIMARY KEY ("id")', script)
        self.assertIn('"id" SERIAL', script)
        self.assertNotIn('"id" SERIAL PRIMARY KEY', script)

    def test_named_identity_pk(self):
        db = self.db

        class Person(db.Entity):
            id = PrimaryKey(int, auto="identity", name="pk_person")
            name = Required(str)

        db.generate_mapping(create_tables=True)
        script = db.schema.generate_create_script()
        self.assertIn('CONSTRAINT "pk_person" PRIMARY KEY ("id")', script)
        self.assertIn('"id" INTEGER GENERATED BY DEFAULT AS IDENTITY', script)
        self.assertNotIn('"id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY', script)
...
class TestIndexOptionsSQLite(unittest.TestCase):
    ...
    def test_named_auto_pk_rejected(self):
        db = self.db
