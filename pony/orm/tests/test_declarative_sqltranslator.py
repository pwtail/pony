...
class TestSQLTranslator(unittest.TestCase):
    ...
    def test_min3(self):
        d = date(2011, 1, 1)
        result = set(
            select(g for g in Grade if min(g.date, d) == d and g.date is not None)
        )
        self.assertEqual(
            result,
            {
                Grade[Student[1], Course["Math", 1]],
                Grade[Student[1], Course["Physics", 2]],
            },
        )
    ...
    @raises_exception(
        TranslationError, "All entities in a query must belong to the same database"
    )
    ...
    def test_add_sub_mul_etc(self):
        result = select(
            s
            for s in Student
            if ((-s.scholarship + 200) * 10 / 5 - 100) ** 2 == 10000 or 5 == 2
        )[:]
        self.assertEqual(result, [Student[2]])
    ...
    @raises_exception(
        TranslationError, "External parameter 'a' cannot be used as query result"
    )
    def test_expr1(self):
        a = 100
    ...
    def test_not_monad1(self):
        result = set(
            select(s for s in Student if not (s.scholarship > 0 and s.name != "S1"))
        )
        self.assertEqual(result, {Student[1]})

    def test_not_monad2(self):
        result = set(
            select(s for s in Student if not not (s.scholarship > 0 and s.name != "S1"))
        )
        self.assertEqual(result, {Student[2], Student[3]})

    def test_subquery_with_attr(self):
        result = set(
            select(s for s in Student if max(g.value for g in s.grades) == "C")
        )
        self.assertEqual(result, {Student[1]})
    ...
    def test_crud_attr_chain(self):
        result = set(
            select(s for s in Student if Group[1].dept.number == s.group.dept.number)
        )
        self.assertEqual(result, {Student[1], Student[2]})
    ...
    def test_composite_key1(self):
        result = set(
            select(
                t
                for t in Teacher
                if Grade[Student[1], Course["Physics", 2]] in t.grades
            )
        )
    ...
    def test_composite_key4(self):
        result = set(
            select(
                s for s in Student if len(c for c in Course if c not in s.courses) == 2
            )
        )
        self.assertEqual(result, {Student[1]})
    ...
    def test_composite_key5(self):
        result = set(
            select(s for s in Student if not (c for c in Course if c not in s.courses))
        )
    ...
    def test_composite_key6(self):
        result = set(
            select(
                c for c in Course if c not in (c2 for s in Student for c2 in s.courses)
            )
        )
    ...
    def test_method_monad(self):
        result = set(
            select(
                s
                for s in Student
                if s not in Student.select(lambda s: s.scholarship > 0)
            )
        )
        self.assertEqual(result, {Student[1]})
    ...
    def test_delete_4(self):
        select(
            g for g in Grade if exists(g2 for g2 in Grade if g2.value > g.value)
        ).delete(bulk=True)
        q2 = select(g for g in Grade)[:]
        self.assertEqual([g.value for g in q2], ["C"])
    ...
    @raises_exception(
        TypeError, "In order to do item assignment, cast QueryResult to list first"
    )
    ...
    @raises_exception(
        TypeError, "In order to do item deletion, cast QueryResult to list first"
    )
    ...
    @raises_exception(
        TypeError, "In order to do append, cast QueryResult to list first"
    )
    ...
    @raises_exception(
        TypeError, "In order to do extend, cast QueryResult to list first"
    )
    ...
    @raises_exception(
        TypeError, "In order to do insert, cast QueryResult to list first"
    )
    ...
    @raises_exception(
        TypeError, "In order to do remove, cast QueryResult to list first"
    )
