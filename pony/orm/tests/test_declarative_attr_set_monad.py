...
class TestAttrSetMonad(unittest.TestCase):
    ...
    def test8(self):
        students = set(
            select(
                s
                for s in Student
                if s.group in (g for g in Group if g.department == 101)
            )
        )
    ...
    def test9(self):
        students = set(
            select(
                s
                for s in Student
                if s.group not in (g for g in Group if g.department == 101)
            )
        )
    ...
    def test20(self):
        groups = set(
            select(
                g
                for g in Group
                if exists(s for s in Student if s.group == g and s.scholarship == 500)
            )
        )
        self.assertEqual(groups, {Group[41], Group[42]})
    ...
    def test21b(self):
        groups = set(select(g for g in Group if not not g.department is not None))
        self.assertEqual(groups, {Group[41], Group[42], Group[43], Group[44]})
    ...
    def test22(self):
        groups = set(
            select(
                g
                for g in Group
                if 700 in (s.scholarship for s in Student if s.group == g)
            )
        )
        self.assertEqual(groups, {Group[42]})
    ...
    def test23b(self):
        groups = set(
            select(
                g
                for g in Group
                if 700 not in (s.scholarship for s in Student if s.group == g)
            )
        )
        self.assertEqual(groups, {Group[41], Group[43], Group[44]})
    ...
    def test29(self):
        students = select(
            g.students.select(lambda s: s.scholarship > 0)
            for g in Group
            if g.department == 101
        )[:]
    ...
    def test30c(self):
        s = Student[2]
        groups = select(
            g for g in Group if g.department == 101 and s in g.students.select()
        )[:]
        self.assertEqual(set(groups), {Group[41]})
    ...
    def test30d(self):
        s = Student[2]
        groups = select(
            g for g in Group if g.department == 101 and s in g.students.filter()
        )[:]
        self.assertEqual(set(groups), {Group[41]})
    ...
    def test31(self):
        ...
        groups = select(
            g
            for g in Group
            if g.department == 101 and g.students.exists(lambda s: s.scholarship > 0)
        )[:]
        self.assertEqual(set(groups), {Group[41]})
