...
class TestDBSession(unittest.TestCase):
    ...
    @raises_exception(
        TypeError,
        "'retry' parameter of db_session must be of integer type. Got: %r" % str,
    )
    ...
    @raises_exception(
        TypeError, "'retry' parameter of db_session must not be negative. Got: -1"
    )
    ...
    @raises_exception(
        TypeError,
        "The same exception ZeroDivisionError cannot be specified "
        "in both allowed and retry exception lists simultaneously",
    )
    def test_retry_8(self):
        @db_session(
            retry=3,
            retry_exceptions=[ZeroDivisionError],
    ...
    def test_retry_9(self):
        ...
        @db_session(
            retry=3, retry_exceptions=lambda e: isinstance(e, ZeroDivisionError)
        )
        def test():
            i = next(counter)
            self.X(a=3, b=3)
    ...
    @raises_exception(
        PonyRuntimeWarning,
        "@db_session decorator with `retry=3` option is ignored for test() function "
    ...
    @raises_exception(
        TransactionError, "Cannot start ddl transaction inside non-ddl transaction"
    )
    ...
    @raises_exception(
        TransactionError,
        "@db_session-decorated test() function with `ddl` option "
...
class TestDBSessionScope(unittest.TestCase):
    ...
    @raises_exception(
        ...
        "Cannot load attribute Student[1].picture: the database session is over",
    )
    def test2(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot load attribute Group[1].major: the database session is over",
    )
    def test3(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot assign new value to Student[1].name: the database session is over",
    )
    def test4(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot load collection Group[1].students: the database session is over",
    )
    def test6(self):
        with db_session:
            g1 = Group[1]
        _length = len(g1.students)

    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot change collection Group[1].students: the database session is over",
    )
    def test7(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot delete object Student[1]: the database session is over",
    )
    def test11(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot change object Student[1]: the database session is over",
    )
    def test12(self):
        with db_session:
            s1 = Student[1]
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot read value of Student[1].name: the database session is over",
    )
