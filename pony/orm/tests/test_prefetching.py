...
class TestPrefetching(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with db_session:
            ...
            Student(
                id=5, name="S5", group=g2, gpa=4.5, biography="S5 bio", courses=[c1, c3]
            )
    ...
    @raises_exception(
        TypeError,
        "Argument of prefetch() query method must be entity class or attribute. Got: 111",
    )
    def test_5(self):
        with db_session:
    ...
    def test_7(self):
        with db_session:
            name, group = (
                select((s.name, s.group) for s in Student)
                .prefetch(Group, Group.major)
                .first()
            )
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot load collection Student[1].courses: the database session is over",
    )
    def test_8(self):
        with db_session:
            s1 = Student.select().first()
    ...
    @raises_exception(
        DatabaseSessionIsOver,
        "Cannot load attribute Student[1].biography: the database session is over",
    )
    def test_11(self):
        with db_session:
            s1 = Student.select().prefetch(Course).first()
...
LIMIT 1"""
    ...
    def test_17(self):
        db.merge_local_stats()
        with db_session:
            q = select(c for c in Course).prefetch(
                Course.students, Student.biography, Group, Group.major
            )
            for c in q:  # 1 query
                for s in c.students:  # 2 queries (as it is many-to-many relationship)
    ...
    def test_19(self):
        db.merge_local_stats()
        with db_session:
            q = Group.select().prefetch(
                Group.students, Student.biography, Student.mentor
            )
            mentors = set()
