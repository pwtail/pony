...
class TestFString(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with db_session:
            Person(
                id=1, first_name="Alexander", last_name="Tischenko", age=23, value=1.4
            )
            Person(
                id=2, first_name="Alexander", last_name="Kozlovskiy", age=42, value=1.2
            )
            ...
            Person(
                id=5, first_name="Musashi", last_name="Miyamoto", age=None, value=0.9
            )
    ...
    def test_1(self):
        ...
        q = select(
            p.id for p in Person if p.first_name + " " + p.last_name == f"{x} {y}"
        )
    ...
    def test_2(self):
        q = select(
            p.id
            for p in Person
            if f"{p.first_name} {p.last_name}" == "Alexander Tischenko"
        )
        self.assertEqual(set(q), {1})

        q2 = select(
            """p.id for p in Person if f'{p.first_name} {p.last_name}' == 'Alexander Tischenko'"""
        )
        self.assertEqual(set(q2), {1})
