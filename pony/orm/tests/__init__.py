...
def _load_env():
    ...
    if provider is None:
        raise ValueError(
            "Incorrect settings pony test db file contents: provider was not specified"
        )
        ...
        if isinstance(x, type) and issubclass(x, unittest.TestCase):
            ...
            def setUpClass(cls):
                raise unittest.SkipTest(
                    "%s tests implemented only for %s provider%s"
                    % (
                        cls.__name__,
                        ", ".join(providers),
                        "" if len(providers) < 2 else "s",
                    )
                )
            ...
            def new_test_func(self):
                if db_params["provider"] not in providers:
                    raise unittest.SkipTest(
                        "%s test implemented only for %s provider%s"
                        % (
                            x.__name__,
                            ", ".join(providers),
                            "" if len(providers) < 2 else "s",
