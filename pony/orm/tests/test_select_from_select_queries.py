...
class TestSelectFromSelect(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with db_session:
            ...
            c2 = Course(
                name="Computer Science", semester=1, credits=20, students=[s2, s3]
            )
    ...
    def test_1(self):  # basic select from another query
        ...
        self.assertEqual(
            db.last_sql.count("SELECT"), 1
        )  # single SELECT...FROM expression
    ...
    @db_session
    @raises_exception(
        ExprEvalError, "`s.scholarship > 0` raises NameError: name 's' is not defined"
    )
    ...
    def test_8(
        self,
    ):  # test using external name which is equal to original query var name
        class Dummy:
            scholarship = 1
    ...
    def test_11(self):  # test .where()
        ...
        q4 = q3.where(
            lambda x: x.age < 24
        )  # the name should be accessible in previous generator
        self.assertEqual(set(s.first_name for s in q4), {"Bruce"})
        self.assertEqual(db.last_sql.count("SELECT"), 1)
    ...
    def test_15(self):  # SELECT ... FROM (SELECT alias.* FROM ...
        q = left_join(
            ...
            for g in Group
            for s in g.students
            if g.number == 123 and s.scholarship > 0
        )
    ...
    def test_18(
        self,
    ):  # SELECT ... FROM (grouped-query returns composite keys), t1 WHERE ...
        q = select(
            (c, count(c.students))
            for c in Course
            if c.semester == 1 and count(c.students) > 1
        )
