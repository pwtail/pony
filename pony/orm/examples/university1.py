...
params = dict(
    ...
    mysql=dict(
        provider="mysql", host="localhost", user="pony", passwd="pony", db="pony"
    ),
    postgres=dict(
        provider="postgres",
        user="pony",
    ...
    oracle=dict(
        provider="oracle", user="c##pony", password="pony", dsn="localhost/orcl"
    ),
)
...
def populate_database():
    ...
    c1 = Course(
        name="Web Design", semester=1, dept=d1, lect_hours=30, lab_hours=30, credits=3
    )
    c2 = Course(
    ...
    s3 = Student(
        name="Chuan Qin", dob=date(1989, 2, 5), gpa=4, group=g101, courses=[c3, c5, c6]
    )
    s4 = Student(
        name="Rebecca Lawson",
        dob=date(1990, 4, 18),
...
def test_queries():
    ...
    students = select(
        s for s in Student if len(c for c in s.courses if c.dept.number == 1) < 4
    )
    print_students(students)
    ...
    students = select(
        s for s in Student if s.group.major == "B.E. in Computer Engineering"
    )
    print_students(students)

    students = select(
        s for s in Student if s.group.dept.name == "Department of Computer Science"
    )
    print_students(students)
