...
# Миграции, application и интроспекция

Поверх провайдера `mysql`/`mariadb` (dialect `MySQL`) работают миграции
(`pony-migrations`), application как логическая группировка (`pony-apps`,
решение 8) и интроспекция через `information_schema` (`pony-introspection`).
В MariaDB нет отдельного понятия схемы — application не создаёт базу данных,
таблицы остаются в текущей БД с обычными именами.
