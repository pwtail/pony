...
11. **Конфиг миграций — `<dir>/config.ini`** (решение пользователя): папка
    миграций (`--dir`, по умолчанию `migrations`) содержит `config.ini` с
    секцией `[database]` и ключом `path = <module>:<attr>`. Спецификация БД
    из `--db` валидируется и всегда перезаписывает `path` в конфиге, поэтому
    повторные вызовы обходятся без `--db`; без `--db` и без конфига — ошибка
    с путём конфига. `config.ini` создаётся вместе с папкой миграций, чужие
    секции и ключи при перезаписи сохраняются, а сам файл не считается
    миграцией.
...
1. **Начальные миграции.** Вместо `generate_mapping(create_tables=True)` —
   ...
   откат (`down`) — не в этом этапе. Спецификация БД из `--db` сохраняется в
   `<dir>/config.ini` (решение 11), поэтому последующие вызовы обходятся без
   `--db`.
